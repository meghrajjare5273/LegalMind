...
export function ChatSidebar() {
  ...
  return (
    <>
      ...
      <TooltipProvider delayDuration={200}>
        ...
        >
          ...
          <div className="mt-3 px-2 flex-1 overflow-y-auto">
            ...
            {/* MESSAGES (static avatars to match design brief) */}
            <SectionLabel className="mt-4" open={open}>
              MESSAGES
            </SectionLabel>
            <div
              className={
                open ? "px-1 space-y-1" : "flex flex-col items-center gap-1"
              }
            >
              {[
                { name: "Esther Howard", src: "/avatars/esther.jpg" },
                { name: "Jacob Jones", src: "/avatars/jacob.jpg" },
                { name: "Cody Fisher", src: "/avatars/cody.jpg" },
              ].map((m) =>
                open ? (
                  <button
                    key={m.name}
                    className="flex items-center gap-3 w-full px-2.5 py-2 rounded-xl hover:bg-gray-50 dark:hover:bg-neutral-800/70"
                  ...
                  <Tooltip key={m.name}>
                    ...
                    <TooltipContent side="right">{m.name}</TooltipContent>
                  </Tooltip>
                )
              )}
            </div>

            {/* RECENT (uses backend chat sessions; retained functionality) */}
            <SectionLabel className="mt-4" open={open}>
              RECENT
            </SectionLabel>
            <div className="px-1">
                  ...
                  {sessions.slice(0, 8).map((s) =>
                    open ? (
                      ...
                      >
                        ...
                        >
                          {/* simple "x" using Plus rotated for minimal deps */}
              ...
              {open ? (
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      aria-label="Account"
                      className="w-full p-1.5 rounded-2xl border bg-white dark:bg-neutral-900 border-gray-200 dark:border-neutral-800"
                    >
      ...
      {/* Mobile Topbar + Drawer (retained from original for responsiveness) */}
      <div className="md:hidden fixed inset-x-0 top-0 h-12 z-30 bg-white/80 dark:bg-neutral-900/70 backdrop-blur-sm border-b border-gray-200 dark:border-neutral-800 flex items-center px-3">
            ...
            <div className="px-4 pb-4 space-y-3 h-full flex flex-col">
              ...
              <div className="pt-2 border-t border-gray-200 dark:border-neutral-800 flex-1 overflow-y-auto">
                {isLoadingSessions ? (
                  ...
                  <div className="space-y-1">
                    <div className="text-xs text-gray-500 dark:text-neutral-400 py-1">
                      Recent Chats
                    </div>
                    {sessions.slice(0, 10).map((s) => (
                      <Link
                        key={s.id}
                        href={`/chat/${s.id}`}
                        className="block px-2 py-2 rounded text-sm hover:bg-gray-50 dark:hover:bg-neutral-800/70"
                      >
        ...
        <div className="ml-2 text-sm">Acme Inc.</div>
